...
export async function GET(request: NextRequest) {
    try {
        ...
        // Handle both Admin SDK (.exists boolean) and Web SDK (.exists() function)
        const docExists = settingsDoc.exists;
        ...
        if (docExists) {
            ...
            // If settings document doesn't exist, assume notifications are disabled.
            console.log('Notification settings document does not exist. Aborting cron job.');
            return NextResponse.json({
                success: true,
                message: "ไม่พบเอกสารการตั้งค่าการแจ้งเตือน"
            });
        }
